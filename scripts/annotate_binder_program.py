    ...
    for g_eid in g_eids:
        try:
            if args.dataset == 'finqa':
                ori_data_item = dataset[g_eid]
                page_title = ori_data_item["id"]
                pre_text = ori_data_item["pre_text"]
                post_text = ori_data_item["post_text"]
                fin_header = ori_data_item["table"][0]
                fin_rows = [ori_data_item["table"][i] for i in range(1, len(ori_data_item["table"]))]
                exe_ans = ori_data_item["qa"]["exe_ans"]
                question = ori_data_item["qa"]["question"]
                g_data_item = {'table': {'page_title': page_title, 'header': fin_header, 'rows': fin_rows},
                           'pre_text': pre_text, 'post_text': post_text, 'question': question, 'answer_text': exe_ans}
            elif args.dataset == 'fetaqa':
                ori_data_item = dataset[g_eid]
                page_title = ori_data_item["table_page_title"]
                feta_header = ori_data_item["table_array"][0]
                feta_rows = [ori_data_item["table_array"][i] for i in range(1, len(ori_data_item["table_array"]))]
                exe_ans = ori_data_item["answer"]
                question = ori_data_item["question"]
                g_data_item = {'table': {'page_title': page_title, 'header': feta_header, 'rows': feta_rows}, 'question': question, 'answer_text': exe_ans}
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='wikitq',
                        choices=['wikitq', 'tab_fact', 'fetaqa', 'finqa'])
