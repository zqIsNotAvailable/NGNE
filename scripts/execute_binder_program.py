...
def worker_execute(
    ...
    for eid, data_item in enumerate(dataset):
        if args.dataset == 'finqa':
            ...
            fin_header = data_item["table"][0]
            fin_rows = [data_item["table"][i] for i in range(1, len(data_item["table"]))]
            exe_ans = data_item["qa"]["exe_ans"]
            question = data_item["qa"]["question"]
            data_item = None
            data_item = {'table': {'page_title': page_title, 'header': fin_header, 'rows': fin_rows},
                         'pre_text': pre_text, 'post_text': post_text, 'question': question, 'answer_text': exe_ans}
        elif args.dataset == 'fetaqa':
            page_title = data_item["table_page_title"]
            feta_header = data_item["table_array"][0]
            feta_rows = [data_item["table_array"][i] for i in range(1, len(data_item["table_array"]))]
            exe_ans = data_item["answer"]
            question = data_item["question"]
            data_item = None
            data_item = {'table': {'page_title': page_title, 'header': feta_header, 'rows': feta_rows},
                         'question': question, 'answer_text': exe_ans}
        eid = str(eid)
        ...
        print("execute one item")
        # score = Evaluator(keys= keys, from_front=0).evaluate(
        #     pred_answer,
        #     gold_answer,
        ...
        #     tuples_list = [score, n_total_scores]
        #
        #     n_total_scores = tuple(sum(x) for x in zip(*tuples_list))
        #     result_tuple = tuple(x / n_total_samples for x in n_total_scores)
        #     print(f'Process#{pid}: Accuracy: {result_tuple}')
...
def main():
    ...
    # if args.dataset == 'wikitq' or args.dataset == 'finqa':
    #     sub = []
    #     for i in range(args.sub_set_start, args.sub_set):
    #         sub.append(dataset[i]['id'])
    #     dataset = [data_item for data_item in dataset if data_item['id'] in sub]
    #
    # if args.dataset == 'fetaqa':
    #     sub = []
    #     for i in range(args.sub_set_start, args.sub_set):
    #         sub.append(dataset[i]['feta_id'])
    #     dataset = [data_item for data_item in dataset if data_item['feta_id'] in sub]
    # Load openai keys
    # if args.dataset == 'fetaqa':
    #     sub = []
    #     for i in range(10):
    ...
    for eid, item in result_dict.items():
        ...
        if args.dataset == 'fetaqa':
            ...
            n_scores = tuple(sum(x) for x in zip(*tuples_list))
    ...
    if args.dataset == 'fetaqa':
        result_tuple = tuple(x / len(result_dict) for x in n_scores)
        print(f'Overall Accuracy: {result_tuple}')
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='wikitq',
                        choices=['wikitq', 'tab_fact', 'finqa', 'fetaqa'])
    ...
    parser.add_argument('--n_processes', type=str, default=3)
    ...
    # parser.add_argument('--sub_set', type=int, default=100)
    # parser.add_argument('--sub_set_start', type=int, default=0)
    # Debugging options
